...
2. Em `app/streamlit_app.py`: importe, adicione o rótulo na lista do
   `st.segmented_control` e o ramo no roteamento.
3. Dados sempre via serviços de `acervo/search/`; consultas repetidas com
   `@st.cache_data(ttl=...)`.

### Mudança de schema do banco

Crie `acervo/persistence/migrations/0004_*.sql` (nunca edite as aplicadas),
...
- **Unitários**: serviços testados com repositórios falsos injetados
  ...
  `test_upload_service.py`); parsers testados com arquivos em `tmp_path`;
  helpers puros da UI em `test_ui_componentes.py`.
